...
int main()
{
    ...
    for(p = 0; p<=1.01; p+=0.1){
        ...
        for(int Nsim = 0; Nsim<50; Nsim++){
            ...
            for (int i = 0; i<n; i++){
                if(recieved[i] == 0){
                    varNode_arr[i]->r = p; // probability that sent 1
                    if(varNode_arr[i]->r == 1) // doing this only to avoid divide by zero
                        varNode_arr[i]->r == 0.99999;
                    if(varNode_arr[i]->r == 0)
                        varNode_arr[i]->r == 0.00001;
                }
            ...
            while(++it<=10){
                ...
                for(int i =0; i<n; i++){
                    varNode_arr[i]->ld1 = (varNode_arr[i]->r)/(1-varNode_arr[i]->r);

                    for(int k = 0; k< dv; k++){
                        varNode_arr[i]->ld1 *= *(varNode_arr[i]->v_r[k]);
                    }

                    for(int j = 0; j<varNode_arr[i]->ind_vr; j++){
                        calc = varNode_arr[i]->ld1/(*(varNode_arr[i]->v_r[j]));
                        varNode_arr[i]->send_cr[j] = calc/(1+calc);
                    }
